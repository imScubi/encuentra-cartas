...
export default async function handler(req, res) {
  ...
  try {
    ...
    const perfilesRes = await fetch(
      `${supabaseUrl}/rest/v1/perfiles?select=id,nombre,plan,plan_vence,mp_preapproval_id&plan=neq.pokeball&plan_vence=gte.${ahora.toISOString()}&plan_vence=lte.${en3dias.toISOString()}`,
      { headers }
    );
    ...
    for (const p of perfiles || []) {
      ...
      const enviados = await notificar(p.id, "Tu plan está por vencer", mensaje, supabaseUrl, headers);
      avisos += enviados;
    }

    // ---- Boosts por vencer ----
    for (const tabla of TABLAS) {
      const select = tabla === "mercado_listings" ? "*,perfiles(id)" : "*,tiendas(perfil_id)";
      const r = await fetch(
...
async function notificar(perfilId, title, body, supabaseUrl, headers) {
