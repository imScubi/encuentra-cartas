    ...
    <div>
      ...
      {loadingAnuncios ? <Loading label="Cargando anuncios..." /> : (
        <>
          ...
          <div className="grid gap-3 mb-8">
            {pendientes.map((n) => (
              <div key={n.id} style={{ background: COLORS.surface, border: `1px solid ${COLORS.surface2}` }} className="rounded-xl p-4 min-w-0">
  ...
  const agregarManual = async (carpetaId) => {
    ...
    try {
      const payload = {
        ...
        set_nombre: manualNuevo.set_nombre || null,
        condicion: manualNuevo.condicion,
        idioma: manualNuevo.idioma,
        precio: Number(manualNuevo.precio),
        cantidad: Number(manualNuevo.cantidad) || 1,
  ...
  return (
    <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.azul}66` }} className="rounded-xl p-4 mb-6 grid gap-3">
      ...
      <p style={{ color: COLORS.muted }} className="text-xs -mt-2">
        Organiza tu inventario en carpetas (álbumes) -- se muestran públicamente como un álbum navegable, página por
        página. Sube una foto de una página completa (rápido) o una foto por cada carta (más lento, pero mucho más
        preciso) y la IA intenta identificar cada carta; o agrégalas del catálogo sin necesidad de fotografiar cada
        una. Tú revisas, le pones precio y publicas.
      </p>
      {error && <ErrorBox message={error} />}
      ...
      <div className="flex gap-2 flex-wrap">
        <input placeholder="Nombre de la carpeta (ej. Álbum Charizards)" value={nombreNueva}
          ...
          style={inputStyle} className="rounded-lg px-3 py-2 text-sm flex-1 min-w-[180px]" />
        <select value={tamanoNueva} onChange={(e) => setTamanoNueva(e.target.value)} style={inputStyle} className="rounded-lg px-2 py-2 text-sm"
...
// Vitrina pública de carpetas: lista de álbumes (nombre + tamaño de página +
// cuántas cartas tiene) y, al abrir uno, una cuadrícula paginada de
// columnas×filas -- como hojear un álbum físico página por página, en vez
// de la lista plana normal del Mercado/tienda. El orden dentro de cada
// página es por fecha en que se agregó la carta (created_at), para que
// coincida con el orden real en que el vendedor la fue guardando.
function CarpetasStorefront({ carpetas, items, tabla, onAbrirDetalle, colorAcento }) {
  ...
  const porPagina = (abierta.columnas || 4) * (abierta.filas || 3);
  const totalPaginas = Math.max(1, Math.ceil(abierta.propias.length / porPagina));
  const visibles = abierta.propias.slice((pagina - 1) * porPagina, pagina * porPagina);

  return (
    <div className="mb-8">
      <button onClick={() => setAbierta(null)} style={{ color: acento }} className="flex items-center gap-1 text-sm mb-3">
      ...
      <div className="grid gap-2 mb-3" style={{ gridTemplateColumns: `repeat(${abierta.columnas || 4}, minmax(0,1fr))` }}>
        ...
        {Array.from({ length: Math.max(0, porPagina - visibles.length) }).map((_, i) => (
          <div key={`vacio-${i}`} style={{ border: `1px dashed ${COLORS.surface2}` }} className="rounded-lg" />
        ))}
      </div>
      {totalPaginas > 1 && (
        <div className="flex items-center justify-center gap-4">
          <button onClick={() => setPagina((p) => Math.max(1, p - 1))} disabled={pagina === 1}
            style={{ color: pagina === 1 ? COLORS.surface2 : COLORS.muted }} className="text-xs font-semibold flex items-center gap-1">
            <ChevronLeft size={14} /> Anterior
          </button>
          <p className="text-xs" style={{ color: COLORS.muted }}>{pagina} / {totalPaginas}</p>
    ...
    <div>
      ...
      {(() => {
        const bloques = {
          ...
          carpetas: vis.carpetas !== false && (
            <div key="carpetas">
              <CarpetasStorefront carpetas={carpetas} items={cartas} tabla="mercado_listings" onAbrirDetalle={onAbrirDetalle} colorAcento={acento} />
            </div>
          ),
        };
...
// reemplaza momentáneamente los banners promocionales (sorteo,
...
function NovedadesCondensadas({ children }) {
  ...
  return (
    <button onClick={() => setAbierto(true)}
      ...
      <Gift size={14} /> Ver sorteos y otras novedades
    </button>
  );
}
...
export default function EncuentraCartas() {
  ...
  const [storeCarpetas, setStoreCarpetas] = useState([]);
  ...
  // usuarios" como en "Buscar" (mismo panel, mismo estado, para que el
  // filtro se sienta como una sola cosa en toda la app en vez de dos
  // paneles distintos). Aparte del filtro por tipo y por TCG (selector
  // global, pasaFiltroTcg). Idioma/estado solo aplican a cartas sueltas; si
  // están activos, un sellado/accesorio (que no tienen esos campos) queda
  // excluido -- tiene sentido, porque filtrar por idioma solo importa si
  // estás buscando cartas.
  const [filtrosAbiertos, setFiltrosAbiertos] = useState(false);
  const [filtros, setFiltros] = useState({ precioMin: "", precioMax: "", idioma: "", condicion: "", zona: "" });
  const filtrosActivos = Object.values(filtros).some(Boolean);
  // La zona vive en columnas distintas según de dónde viene el resultado:
  // plana en mercado_listings (item.zona), anidada bajo tiendas en
  // inventario_tienda/sellado_tienda (item.tiendas.zona).
  const zonaDe = (item) => item.zona || item.tiendas?.zona || "";
  const pasaFiltros = (item) => {
    const f = filtros;
  ...
  const [ordenResultados, setOrdenResultados] = useState(""); // "" | "precio_asc" | "precio_desc"
  const ordenarPorPrecio = (arr) => {
    if (ordenResultados === "precio_asc") return [...arr].sort((a, b) => Number(a.precio) - Number(b.precio));
    if (ordenResultados === "precio_desc") return [...arr].sort((a, b) => Number(b.precio) - Number(a.precio));
    return arr;
  };
  ...
  const openStore = (store) => {
    ...
    setStoreResenas([]); setStoreVentasCompletadas(0); setStoreDestellos(0); setStoreSeguidores(0); setStoreCarpetas([]);
    ...
    sb(`carpetas?select=id,nombre,columnas,filas&tienda_id=eq.${store.id}&order=created_at.desc`).then(setStoreCarpetas).catch(() => {});
    if (store.perfil_id) {
      ...
      <main style={{ position: "relative", zIndex: 1 }} className="max-w-5xl mx-auto px-4 sm:px-8 pt-10 pb-24 sm:py-10">
          ...
          <div>
            ...
            {!searching && query.trim() && (
              <div className="max-w-3xl mx-auto mb-4">
                ...
                {filtrosAbiertos && (
                  <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.surface2}` }} className="rounded-xl p-4 mt-3 grid sm:grid-cols-5 gap-2">
                    <input placeholder="Precio mín." type="number" value={filtros.precioMin}
                      onChange={(e) => setFiltros((f) => ({ ...f, precioMin: e.target.value }))}
                      style={{ background: COLORS.bg, color: COLORS.text, border: `1px solid ${COLORS.surface2}` }} className="rounded-lg px-2 py-2 text-sm" />
                    <input placeholder="Precio máx." type="number" value={filtros.precioMax}
            ...
            {(() => {
              const selladoVisible = ordenarPorPrecio(searchResults.sellado.filter(pasaFiltroTcg).filter(pasaFiltros));
              const tiendasVisibles = ordenarPorPrecio(searchResults.tiendas.filter(pasaFiltroTcg).filter(pasaFiltros));
              const mercadoVisible = ordenarPorPrecio(searchResults.mercado.filter(pasaFiltroTcg).filter(pasaFiltros));
              return (
                <>
                  {!searching && query.trim() && tiendasVisibles.length === 0 && mercadoVisible.length === 0 && selladoVisible.length === 0 && (
                    <p style={{ color: COLORS.muted }} className="text-center py-16 text-sm">
                      {filtrosActivos && (searchResults.tiendas.length + searchResults.mercado.length + searchResults.sellado.length) > 0
                        ? "Nadie tiene algo así con esos filtros. Prueba quitando alguno."
        ...
        {view === "news" && (
          <div>
            ...
            <div className="grid gap-4">
              {news.map((n) => (
                <div key={n.id} style={{ background: COLORS.surface, border: `1px solid ${COLORS.surface2}` }} className="rounded-xl p-5 min-w-0">
                  <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
                    <div className="flex items-center gap-2">
                      <AvatarImg url={n.tiendas?.perfiles?.avatar_url} size={24} />
        ...
        {view === "storeDetail" && selectedStore && (
          <div>
            ...
            {loadingStoreDetail ? <Loading label="Cargando inventario..." /> : (
              <>
                {/* sellado_tienda no tiene carpeta_id (migración 019 solo la agregó a
                    inventario_tienda/mercado_listings) -- una carpeta solo agrupa cartas sueltas. */}
