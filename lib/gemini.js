...
// Variante de solo texto (sin imagen) -- para el análisis en palabras
// simples del boletín semanal de precios (ver api/cron/recordatorios.js).
export async function llamarGeminiTexto(apiKey, modelo, prompt) {
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${modelo}:generateContent?key=${apiKey}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function llamarGeminiTextoConReintento(apiKey, prompt, modeloPreferido) {
  const candidatos = await elegirModeloGemini(apiKey, modeloPreferido);
  let ultimoError = null;
  for (const modelo of candidatos) {
    try {
      return await llamarGeminiTexto(apiKey, modelo, prompt);
    } catch (e) {
      ultimoError = e;
